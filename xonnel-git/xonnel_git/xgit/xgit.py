...
class XCmd:
    @staticmethod
    def exec(cmd:str="", cwd:str=None, check:bool=True):
...
class XGit:
    def __init__(self, name:str="whoisthereitsme", repo:str="xonnel-xonnel", branch:str="main", path:str|Path=None):
        self.name = name
        self.repo = repo
        self.branch = branch
        self.path = Path(path)
    ...
    def post(self):
        XCmd.exec(f"git init {self.path}", cwd=self.path)
